...
// - Uruchamia funkcję, która wczytuje zmienną typu string i oblicza liczbę wystąpień znaków niebędących literami 
...
int liczbawystapienliter(string tekst){
    ...
    for(int i=0; i<tekst.length(); i++){ //petla przechodzaca przez caly tekst
        if((tekst[i]<65) || (tekst[i]>90 && tekst[i]<97) || (tekst[i]>122)) licznik++; //jesli znak nie jest litera to zwieksz licznik
    } // ! neguje warunek
    return licznik;
}
